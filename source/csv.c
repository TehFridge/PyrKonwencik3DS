...
static int read_next_field(FILE *fp, char field_delim, char text_delim, CSV_FIELD *field)
{
    char ch = 'a';
    bool done = false;
    bool in_text = false;
    ...
    while (!done) {
        ...
        if (ch == EOF) {
            done = true;  // End of file reached
            break;
        }

        if (!in_text) {
            if (ch == text_delim) {
                in_text = true;  // Entering a quoted text field
            } else if (ch == field_delim || ch == '\n') {
                done = true;  // Delimiter or newline ends the field
            } else {
                add_char(&tmp, &c, ch);  // Regular character in a field
            }
        } else {
            if (esc) {
                if (ch == text_delim) {
                    add_char(&tmp, &c, text_delim);  // Handle embedded quotes
                    esc = false;
                } else {
                    // Invalid escape, treat as end of field
                    ungetc(ch, fp);  // Put back unexpected character
                    done = true;
                }
            } else {
                if (ch == text_delim) {
                    esc = true;  // Handle escape sequence for quotes
                } else if (ch == '\n') {
                    // Ignore newline inside quoted field
                    add_char(&tmp, &c, ch);  // Continue adding newline to field
                } else {
                    add_char(&tmp, &c, ch);  // Regular character inside quotes
                }
            }
        }
    }

    // Logging parsed field
    printf("[read_next_field] Parsed field: '%s'\n", tmp);

    if (field != NULL) {
        set_field(field, tmp);  // Store the field in the provided CSV_FIELD struct
    }
    ...
    // Handle the move to the next field or end of file
    fpos_t pos;
    int retval;
    done = false;
    while (!done) {
        ch = getc(fp);
        if (ch == field_delim) {
            retval = 0;  // Another field, continue
            done = true;
        } else if (ch == '\n') {
            // End of row or file
            fgetpos(fp, &pos);
            ch = getc(fp);
            if (ch == EOF) {
                retval = 2;  // End of file
            } else {
                retval = 1;  // Next line starts
            }
            fsetpos(fp, &pos);
            done = true;
        } else if (ch == EOF) {
...
int csv_load(CSV_BUFFER *buffer, char *file_name)
{
    ...
    while (!end) {
		...
		printf("[csv_load] Read result: %d (row %d, col %d)\n", next, i, j);
		if (i > 1000) {
			printf("Too many rows; possible malformed field\n");
			break;
		}
    }
